...
if [ ${generate} -eq 0 ]; then
    ...
    docker ${BUILDX} --no-cache -t he0119/typecho:${version}-php${TAG} --build-arg TAG=${TAG} --build-arg URL=${URL} --build-arg CONFIG="${CONFIG}" --build-arg PHP_EXTENSION="${PHP_EXTENSION}" --build-arg PHP8_SOCKETS_WORKAROUND="${PHP8_SOCKETS_WORKAROUND}" .
    ...
    if [[ ${buildx} -eq 0 && ${upload} -eq 1 ]]; then
        docker push he0119/typecho:${version}-php${TAG}
    fi
